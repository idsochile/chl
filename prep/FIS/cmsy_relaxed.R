...
runCMSY<-function(cdat, stockNumber){
  ...
  #Init of Lines to be included when you use resilience
  if(res == "Very low"){ start_r <- c(0.015, 0.1) } else { if(res == "Low"){ start_r <- c(0.05,0.5) } else {
    if(res == "High"){ start_r <- c(0.6,1.5)
    } else { start_r <- c(0.1,1) } } }
  #End of lines to be included if you use resilience
  ...
  finalbio <- if(ct[nyr]/max(ct) > 0.5) {c(0.4,0.8)} else {c(0.01,0.8)} # Relaxed prior
